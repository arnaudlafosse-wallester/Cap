...
export async function startAiGeneration(
	...
	try {
		...
		const transcript = await fetchTranscript(videoId, userId, bucketId);

		if (!transcript) {
			// Mark as skipped if no transcript
			await db()
				.update(videos)
				.set({
					metadata: {
		...
		console.log(`[generateAi] AI generation completed for video ${videoId}`);
